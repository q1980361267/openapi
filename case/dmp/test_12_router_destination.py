...
class Test_RouterDestination(unittest.TestCase):
    """目的地配置接口"""
    ...
    def test_01(self):  # 执行逻辑::设置入参，参数正确填写
        ...
        logging.info(f"case:新增路由目的地端点-成功\n请求地址：{r.url}\t请求方式:{r.request.method}\n"
                     f"请求头：{r.request.headers}\n请求正文：{parse.unquote(r.request.body)}\n响应头：{r.headers}\n响应正文：{r.text}\n")
    ...
    def test_02(self):  # 执行逻辑::设置入参，参数正确填写
        ...
        logging.info(f"case:修改路由目的地端点-成功\n请求地址：{r.url}\t请求方式:{r.request.method}\n"
                     f"请求头：{r.request.headers}\n请求正文：{parse.unquote(r.request.body)}\n响应头：{r.headers}\n响应正文：{r.text}\n")
    ...
    def test_03(self):  # 执行逻辑::设置入参，参数正确填写
        ...
        logging.info(f"case:获取路由目的地端点-成功\n请求地址：{r.url}\t请求方式:{r.request.method}\n"
                     f"请求头：{r.request.headers}\n请求正文：{parse.unquote(r.request.body)}\n响应头：{r.headers}\n响应正文：{r.text}\n")
    ...
    def test_04(self):  # 执行逻辑::设置入参，参数正确填写
        ...
        logging.info(f"case:分页查询路由目的地端点-成功\n请求地址：{r.url}\t请求方式:{r.request.method}\n"
                     f"请求头：{r.request.headers}\n请求正文：{parse.unquote(r.request.body)}\n响应头：{r.headers}\n响应正文：{r.text}\n")
